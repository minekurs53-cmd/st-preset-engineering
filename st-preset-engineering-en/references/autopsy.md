...
| §4 Orphan table | In prompts[] but not in order | Orphan = the feature doesn't exist (invisible/untoggleable); list functional orphans separately from junk orphans; **data-container entries** (unusually large char count, body is serialized JSON, e.g. `SPreset配置` = an extensions copy) listed separately and marked "do not wire back"; **reverse reconciliation**: order referencing identifiers absent from prompts[] (stale refs) is listed too — the classic residue of a hot-upgrade rename/version swap (measured: two order blocks each carrying 2 old-version identifiers while the current entries became orphans) |
...
**Law of three copies**: when the same data (e.g., the same-named regexes) appears in multiple places, first determine at runtime which copy is effective (the binding layer is inert without the extension), **change only the effective layer, sync or delete the rest — never maintain both in parallel**. Reconciliation goes through Card 5 §5's copy-layer reconciliation lines (only-native / only-copy / same-name diffs) — never by eye.
...
- **Zone/group recognition must be style-agnostic**: at least six real styles exist — 〈…〉 heading entries, paired boundary entries (——X begin——/——X end——), same-name boundary pairs (toggle type: ——X—— sharing the name at open and close; second occurrence = close), emoji+category-name prefixes, HTML-tag paired entries (`<background>…</background>`), "----X" hyphen headings. **The same skill will meet mutually incompatible zone families**: built-in style recognition is untrustworthy; the profile's `zone_title_re`/`zone_pair_start_re`/`zone_pair_end_re`/`zone_category_re` data-driven rules **take precedence over built-ins**; if the profile has none and recognition fails → switch to a targeted-extraction probe; never build on a wrong recognition. (All six forms appeared across test runs (the sixth = same-name pairs, added this round): 〈〉/——pairs/same-name pairs/HTML pairs/emoji prefixes/━━ separator headings; the emoji-prefix and ━━ separator classes needed `zone_category_re` and the "capture group 1 = zone name" semantics — see profile-spec.)
...
- **Three identifier forms** (classify before auditing; the root cause of reference-scan false positives):
  ...
  2. **ST built-in slot names** ('main'/'nsfw'/'jailbreak'/'scenario'/'charPersonality'/'personaDescription'/'worldInfoBefore'…; full list in the local ST source's built-in identifier table in PromptManager) — carry runtime fill semantics; even renamed/repurposed they are still ST-filled (presumed); never treat as ordinary short words; an incomplete built-in list in a tool mislabels these slots as "custom sp=true" (measured: personaDescription was once missed)
...
- [ ] Six reconciliations quantified: orphans / order stale refs / dead slots / double-False / mutex violations / remote dependencies

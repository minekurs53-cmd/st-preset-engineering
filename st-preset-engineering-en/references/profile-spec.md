...
- None found: build the skeleton fields from the "minimal profile" below; S0/S1 fill in most of the structure section
- A profile-referenced file/command fails to run: **report explicitly which reference failed** — never silently skip, never guess a substitute
...
```jsonc
{
  "preset": {
    ...
    "tool_dir": "<tool_dir>",                        // tool directory (relative to repo root or absolute)
...
```
S0 fills hash_baseline; after S1's `p1 --preset`, most of structure/complexity is filled; **when group_notation or zone_marker_style can't be determined, ask the user face to face**.

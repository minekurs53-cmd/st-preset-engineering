...
|---|---|---|---|
| L1 | **Self-serving documentation**: writing docs about the work replaces the work; needing "docs about managing docs" = documentation surplus | 4 of 13 sessions went mostly to maintaining consistency of a 420k-character documentation corpus | SKILL.md hard cap ≤300 lines; routing table controls pipeline weight; drift self-check question 1 |
| L2 | **Self-verification**: imagined models + sub-agent roleplay + self-written asserters, conclusions passed on as "verified" | 3 "verification" sessions were all invalid, overturned by the 8th session | Discipline 2; L4 externalized to the user; assertion applicability boundaries mandatory |
...
| L4 | **Focus drift**: sliding from "usable by the user" toward "process rigor / tooling completeness", pulled back by the user twice | Session 8: "right direction, wrong focus"; session 13: "feels like a loop" | Drift self-check question 4; the routing table hard-codes the minimal correct path |
| L4b | **Control-experiment inertia**: "get baseline data first" becomes a procrastination excuse; a fix backed by measured direction should run in parallel with effect comparison | Session 12 produced v2 then defaulted to waiting for the user's 10 turns — the user pointed out "the original preset fluctuates too" | After S3, branch immediately: fixes supported by data → S4; genuine open questions → unverified list |
...
| L8 | **The source code is right there**: the most important question stays "blocked, need leads" while the answer sits 15 minutes away in local source | I1 marked "blocked" for three sessions | Any "what does ST actually do" question: first action is reading local ST source (`public/scripts/` — macros / openai / regex engine / PromptManager are proven forensics points), evidence rank equal to official docs |
